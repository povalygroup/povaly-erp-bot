...
3. **Verify activation**:
   - You should see the welcome message above
   - You'll now receive DM notifications for task updates, approvals, and alerts
   - Test it by typing `/mytasks` in the group
...
**Important Note About Pova:**
- Pova can only run in ONE location at a time
- If the system is running on the server, it won't work on local machines
- If you see Pova not responding, it may be running elsewhere
- Contact your administrator if Pova stops responding

### Step 3: Understand Your Role

Your access level determines what you can do in the system. Pova has 4 user roles:

- **👤 Regular User** (Default) - Create tasks, submit work, report issues, track attendance
- **🎯 QA Reviewer** - All regular permissions + approve/reject quality reviews
- **👔 Manager** - All QA permissions + approve leave, view team workload, bulk assign tasks
- **👑 Administrator** - Full system access + manage users, edit messages, system configuration

**Note:** Your role is configured by administrators. Contact your manager if you need different permissions.
...
**Who can post:** Everyone (all users can create tasks)  
**What you do:**
- Create new tasks with `/newtask`
...
**How it works:**
- Anyone can create tasks using `/newtask` command
...
**1. Check-in (Automatic or Manual)**
- When you start your first task of the day, Pova automatically checks you in
- If you arrive after 11:00 AM, you'll be marked as late
